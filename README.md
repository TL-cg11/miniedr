...
Windows 환경용 경량 EDR — YARA 룰과 해시 블랙리스트로 파일을 실시간 탐지하고 자동 격리 프로젝트
...
지정한 폴더를 백그라운드 스레드로 감시
파일 생성·수정 이벤트가 발생하면 자동으로 검사
종료 시 콘솔에 `stop` 을 입력
...
다른 콘솔에서 `C:\\test\\watch\\` 안에 EICAR 파일을 떨어뜨리면 즉시 탐지·격리
...
\- 단일 폴더 감시 — Monitor 모드는 한 번에 하나의 폴더만 감시
...
\*이 프로젝트는 EDR 의 핵심 동작 원리를 학습하기 위한 프로젝트 결과물입니다.\*
